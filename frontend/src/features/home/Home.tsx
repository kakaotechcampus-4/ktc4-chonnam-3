...
export default function Home() {
  ...
  const errorReason = isApiError(homeQuery.error) ? homeQuery.error.error.reason : undefined;
  const githubTokenInvalid =
    errorReason === 'github_token_invalid' || errorReason === 'token_invalid';
    ...
    <div className="flex min-h-screen flex-col bg-paper text-ink">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-10">
        <h1 className="text-lg font-bold">
          {`안녕하세요, ${identity?.name ?? home?.name ?? ''} 님!`}
        </h1>

        {identity && !identity.githubLinked && !githubTokenInvalid && (
          <p className="mt-4 text-sm text-muted">GitHub 연결을 확인해주세요.</p>
        )}
        {githubTokenInvalid && (
