...
- 원본: [루트 지침](../../../CLAUDE.md), [AI 아키텍처](../architecture.md), [BE 아키텍처](../../backend/architecture.md). 후속 결정·미완료 작업은 [결정 목록](../decisions/README.md)과 [구현·검수 인계표](../../../ai/docs/pipeline.md#기존-id별-구현검수-인계)를 따른다.
...
후속 범위 갱신(2026-09-22): [0019](../decisions/0019-sprint1-profile-role-summary-restoration.md)에서 개인 역할의 LLM 요약을 기존 구현 대상으로 복원했다. 위 비호출 기준은 Wanted 변환·프로필 통계 집계에 적용하며, 역할 요약은 기존 `backend/app/llm_tasks/profile_summary.py` 경계에서 구현한다. 이번 결정으로 패키지 이동·별도 모듈·job을 추가하지 않는다.
...
구조 구현 중 기존 파일의 사용자 변경이나 이 설계에 없는 계약 변경이 필요하면 해당 부분만 재검토한다. 코드 배치 승인으로 작성 당시 `later.md`에 남긴 모델·schema·점수·운영 정책까지 승인된 것으로 확대하지 않는다. 해당 항목의 후속 결정은 [결정 목록](../decisions/README.md)에서 확인한다.
