...
기본 실행은 실제 백엔드로 연결한다. mock은 개발 환경에서 명시적으로 켰을 때만 동작한다.

화면 미리보기를 하려면 `frontend/.env.local`(git 추적 제외)에 다음을 설정하고 개발 서버를 다시 시작한다.

```
VITE_USE_MSW=true
```

실제 GitHub 로그인·쿠키·토큰 갱신을 검증할 때는 이 값을 제거하거나 `false`로 설정하고 개발 서버를 다시 시작한다. mock의 `/me`는 로그인된 사용자 fixture를 반환하므로 `/login` 접근이 `/home`으로 이동할 수 있다. 이는 화면 미리보기이며 실제 인증 성공을 뜻하지 않는다.

mock 로그아웃도 고정된 `/me` 응답을 바꾸지 않으므로 홈으로 돌아온다. 실제 로그아웃과 세션 폐기 여부는 mock을 끈 상태에서 검증한다.

## 4. 핸들러 확인하기

### 워커가 붙었는지

`VITE_USE_MSW=true`로 `npm run dev`를 실행한 후 브라우저 콘솔에 두 줄이 보이면 정상이다.
...
등록에 실패하면 콘솔의 실패 사유를 확인한다. mock이 동작하지 않는 `/api` 요청은 Vite 프록시를 통해 실제 백엔드로 전달되므로 mock 검증 성공으로 간주하지 않는다.
