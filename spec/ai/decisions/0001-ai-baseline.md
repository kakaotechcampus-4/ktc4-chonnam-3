...
후속 안내(2026-09-22): 아래 7번의 모델 선택과 공급자·API 모델 ID의 미결정 상태는 [0011 결정](0011-sprint1-model-selection.md)으로 대체됐다. 현행 Sprint 1 공급자·API 모델 ID는 OpenAI `gpt-5.6-luna`이며, 실제 연결·계정 접근·task별 품질 검증은 별도 구현 작업이다. 아래 본문은 작성 당시 이력으로 보존한다.
