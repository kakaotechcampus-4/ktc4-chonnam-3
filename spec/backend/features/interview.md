...
- 9턴의 persona별 횟수는 `tech_lead` 6턴, `domain_lead` 2턴, `hr_manager` 1턴으로 고정한다. 첫 질문은 `hr_manager` 1턴에 포함한다.
- 2턴부터 Director가 persona별 남은 횟수 안에서 선택한다. 첫 질문 이후의 persona 순서는 고정하지 않는다.

[현재 로컬 기준 채택 결정](../../shared/decisions/0002-local-policy-baseline.md)에 따른 확정 정책이며, Director의 횟수 제어 구현·검증은 별도 작업이다.
