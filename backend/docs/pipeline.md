...
막히는 주체는 워커가 아니라 API 다. 유니크 인덱스는 Postgres 제약이라 Redis · 워커와 무관하고,
새 요청은 enqueue 단계까지 가지도 못한다.
...
두 번째에는 미결이 있다. `running` 으로 남은 행이 **정말 실행 중인지, 워커가 죽어서 남은 것인지**
구분해야 한다. 워커 하트비트와 lease/lock TTL 중 어느 쪽으로 갈지는 미결이다.
...
**구독이 먼저다.** 반대로 하면 상태를 읽은 뒤 구독하기 전 사이에 run 이 끝났을 때 `completed` 를
영영 못 받는다.
...
SSE 는 프록시에서 버퍼링되면 죽는다. Caddy 의 `/api/*` 라우트에는 FE 정적 파일용 압축·캐시
설정을 섞지 않는다([deploy.md](deploy.md) 3절 ③). 응답 헤더는 `Cache-Control: no-store`,
15초 주기 keep-alive 코멘트(`: ping`)를 보낸다.
...
WS ping 주기는 프록시·네트워크 유휴 타임아웃보다 짧아야 한다. `ws:lock` 60초 하트비트를 그대로
쓰면 경계에서 겹칠 수 있으므로 [deploy.md](deploy.md) 3절 ④ 에서 같이 조정한다.
